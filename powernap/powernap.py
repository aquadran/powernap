...
from monitors import ProcessMonitor, PingMonitor, LockMonitor, LoadMonitor, InputMonitor, TCPMonitor, UDPMonitor, IOMonitor, WoLMonitor, ConsoleMonitor, DiskMonitor
...
class PowerNap:
    ...
    def load_monitors_config(self, monitor, items):
        ...
        if monitor == "PingMonitor":
            self.MONITORS.append({"monitor":monitor, "name":items[0], "ip":items[1], "absent":self.ABSENT_SECONDS})
        if monitor == "LockMonitor":
            self.MONITORS.append({"monitor":monitor, "name":items[0], "lock":items[1], "absent":self.ABSENT_SECONDS})
    ...
    def get_monitors(self):
        ...
        for config in self.MONITORS:
            ...
            if config["monitor"] == "PingMonitor":
                p = PingMonitor.PingMonitor(config)
            if config["monitor"] == "LockMonitor":
                p = LockMonitor.LockMonitor(config)
